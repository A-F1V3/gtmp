...
func parseChal(b []byte, peerKey []byte, key []byte) (dig []byte, err int) {
	if b[0] != 0x3 {
		log.Printf("handshake: invalid rtmp version")
		err = 1
		return
	}
	...
	epoch := b[1:5]
	ver := b[5:9]
	log.Printf("handshake:   epoch %v ver %v", epoch, ver)
	...
	log.Printf("handshake:   offs = %v", offs)
...
func handShake(rw io.ReadWriter) {
	...
	log.Printf("handshake: send server chal")
	rw.Write(b)
